...
void CustomStepButton::paintButton(juce::Graphics& g, bool isMouseOverButton, bool isButtonDown)
{
    ...
    // Draw sequencer highlight if active
    if (sequencerActive)
    {
...
{
    this->inactiveImage = std::move(inactive);
    this->activeImage = std::move(active);
    repaint();
}
